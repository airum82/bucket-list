...
app.post('/api/v1/bucket-items', (request, response) => {
  const item = request.body;
  if(Object.keys(item).length !== 2) {
    return response.status(422).json('Invalid Format')
  }
  database('list_items').insert(item, 'id')
    ...
    .catch(err => {
      return response.status(422).json('Missing title or description or invalid format')
    })
});
